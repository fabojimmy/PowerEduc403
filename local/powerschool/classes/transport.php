...
class transport extends moodleform {
    ...
    public function definition() {
        ...
        $user=$DB->get_records_sql("SELECT u.id,firstname,lastname FROM {user} u,{role_assignments} r WHERE r.roleid=9 AND r.userid=u.id");

        $mform->addElement('header','inscription', 'Transportation');
        ...
        foreach ($user as $key => $value) {
            $userta[$key]=$value->firstname."-".$value->lastname;
        }
        $mform->addElement('text', 'matricule', 'Matricule Transport'); // Add elements to your form
        $mform->setType('matricule', PARAM_TEXT);                   //Set type of element
        $mform->addRule('matricule', 'Enregistrer le libellé du matricule', 'required', null, 'client');
        $mform->addHelpButton('matricule', 'specialite');
       
        $mform->addElement('text', 'marque', 'Marque'); // Add elements to your form
        $mform->setType('marque', PARAM_TEXT);                   //Set type of element
        $mform->addRule('marque', 'Enregistrer le libellé de la marque', 'required', null, 'client');
        $mform->addHelpButton('marque', 'specialite');
       
        $mform->addElement('text', 'place', 'Place'); // Add elements to your form
        $mform->setType('place', PARAM_TEXT);                   //Set type of element
        $mform->addRule('place', 'Enregistrer le nombre de la place', 'required', null, 'client');
        $mform->addHelpButton('place', 'specialite');
     
        $mform->addElement('select', 'idconducteur', 'Conducteur',$userta); // Add elements to your form
        $mform->setType('idconducteur', PARAM_TEXT);                   //Set type of element
        $mform->addRule('idconducteur', 'Enregistrer le conducteur', 'required', null, 'client');
        $mform->addHelpButton('idconducteur', 'specialite');
        ...
        $mform->addElement('textarea', 'description', 'Information sur le transport','wrap="virtual" rows="2" cols="2"' ); // Add elements to your form
    ...
    public function get_transport(int $transportid)
    {
        global $DB;
        return $DB->get_record('transport', ['id' => $transportid]);
    }
    public function update_transport($data): bool
    {
        global $DB;
        // global $USER;
        // $object = new stdClass();
        ...
        return $DB->update_record('transport', $data);
    }
}