<?php
   ...
   if ($_POST["cycle"] && $_POST["specialite"]) {
    $sql="SELECT c.id as idco,c.fullname FROM {coursspecialite} cs,{cycle} as cy,{course} as c,{courssemestre} css,{affecterprof} af 
    WHERE idspecialite='".$_POST['specialite']."' AND idcycle='".$_POST['cycle']."' AND idcourses=c.id AND cs.id=css.idcoursspecialite 
    AND css.id=af.idcourssemestre AND idprof='".$USER->id."'";
