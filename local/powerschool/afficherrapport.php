...
$sql = "SELECT * FROM {rapportcours} WHERE idcampus ='".ChangerSchoolUser($USER->id)."' AND idpro='".$_GET["idprof"]."' AND idcours='".$_GET["idcours"]."' AND id='".$_GET["idrap"]."'";
...
foreach ($salles as $key => $value) {
  $value->duree=($value->duree/(60*60*1000));
}
...
echo $OUTPUT->render_from_template('local_powerschool/afficherrapport', $templatecontext);
