...
if ($mform->is_cancelled()) {
    ...
    redirect($CFG->wwwroot . '/local/powerschool/tranche.php', 'annuler');

} else if ($fromform = $mform->get_data()) {

$recordtoinsert = new tranche();

    if($fromform->id) {

        $recordtoinsert->update_tranche($fromform->id, $fromform->libelletranche );
...
if ($id) {
    ...
    $specialite = $newspecialite->get_tranche($id);
    if (!$specialite) {
