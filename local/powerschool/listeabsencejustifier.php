...
$PAGE->set_url(new moodle_url('/local/powerschool/listeabsencejustifier.php'));
$PAGE->set_context(\context_system::instance());
$PAGE->set_title('liste absences justifier');
$PAGE->set_heading('liste absences justifier');

$PAGE->navbar->add('Administration du Site', $CFG->wwwroot.'/admin/search.php');
$PAGE->navbar->add(get_string('listeabsencejustifier', 'local_powerschool'), $managementurl);
// $PAGE->requires->js_call_amd('local_powerschool/confirmsupp');
// $PAGE->requires->js_call_amd('local_powerschool/confirmsupp');



$sql='SELECT firstname,lastname,j.description,j.timecreated FROM {user} u,{absenceetu} a,{absencejustifier} j
      WHERE u.id=a.idetudiant AND j.idabsenceetu=a.id';

$listejustifier=$DB->get_records_sql($sql);



foreach ($listejustifier as $key ){

    $time = $key->timecreated;

    $dated = date('d-m-Y',$time);

    $key->timecreated = $dated;
...
$templatecontext = (object)[
    'justifier' => array_values($listejustifier),
...
// {
    ...
    echo $OUTPUT->render_from_template('local_powerschool/listeabsencejustifier', $templatecontext);
// }
// else{
//     \core\notification::add("Vous avez pas autorisation", \core\output\notification::NOTIFY_ERROR);
