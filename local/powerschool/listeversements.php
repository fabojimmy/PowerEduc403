...
{}
$sqlsommeetufilsp="SELECT i.id,firstname,lastname,libellecycle,libellespecialite  FROM {inscription} i,{paiement} pa,{user} u,{cycle} cy,{specialite} sp
                   WHERE u.id=i.idetudiant AND i.id=pa.idinscription AND cy.id=i.idcycle AND sp.id=i.idspecialite AND idspecialite ='".$_GET["idsp"]."' 
                   AND idcycle='".$_GET["idcy"]."' AND i.idanneescolaire='".$_GET["idan"]."'";
...
echo $OUTPUT->render_from_template('local_powerschool/listeversements', $templatecontext);
